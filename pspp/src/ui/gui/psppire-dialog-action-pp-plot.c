...
static GObject *psppire_dialog_action_pp_plot_constructor (GType, guint, 
																						GObjectConstructParam *);

static void psppire_dialog_action_pp_plot_finalize (GObject *);

static void
psppire_dialog_action_pp_plot_class_init (PsppireDialogActionPPPlotClass *class);

G_DEFINE_TYPE (PsppireDialogActionPPPlot, psppire_dialog_action_pp_plot, PSPPIRE_TYPE_DIALOG_ACTION);

static gboolean
dialog_state_valid (gpointer rd_)
{ 
 PsppireDialogActionPPPlot *rd = PSPPIRE_DIALOG_ACTION_PP_PLOT (rd_);
...
static void
pp_plot_estimate_data_toggled (GtkCheckButton *estimate_from_data_checkbox, PsppireDialogActionPPPlot *rd)
{
	if (gtk_toggle_button_get_active(GTK_CHECK_BUTTON (rd->estimate_from_data_checkbox)))
		{
			gtk_widget_set_sensitive  (rd->parameter1_entry, FALSE);
...
static void
psppire_dialog_action_pp_plot_activate (PsppireDialogAction *a)
{        
}

GObject * psppire_dialog_action_pp_plot_constructor(GType 									type, 
																					guint           				n_properties, 
																					GObjectConstructParam *properties)
{
	PsppireDialogActionPPPlot *act;
  ...
  GtkCellRenderer *renderer;
	...
	obj = G_OBJECT_CLASS(psppire_dialog_action_pp_plot_parent_class)->constructor (type,
																														n_properties,properties);
	act = PSPPIRE_DIALOG_ACTION_PP_PLOT (obj);
	pda = PSPPIRE_DIALOG_ACTION (obj);
	
	GtkBuilder *xml = gtk_builder_new_from_file ("pp-plot.ui");
	
	GHashTable *thing = psppire_dialog_action_get_hash_table (pda);
  g_hash_table_insert (thing, act, xml);
	
  pda->dialog = get_widget_assert (xml, "pp-plot-dialog");
  ...
  gtk_entry_set_input_purpose(GTK_ENTRY (act->parameter2_entry), GTK_INPUT_PURPOSE_NUMBER);
	...
	g_signal_connect (act->estimate_from_data_checkbox, "toggled", G_CALLBACK (pp_plot_estimate_data_toggled), act);
...
generate_syntax (const PsppireDialogAction *a)
{
  ...
  PsppireDialogActionPPPlot *act = PSPPIRE_DIALOG_ACTION_PP_PLOT (a);
  gchar *text;
  GString *string = g_string_new ("GRAPH /PP");
...
psppire_dialog_action_pp_plot_class_init (PsppireDialogActionPPPlotClass *class)
{
  psppire_dialog_action_set_activation (class, psppire_dialog_action_pp_plot_activate);

  PSPPIRE_DIALOG_ACTION_CLASS (class)->generate_syntax = generate_syntax;
  
  GObjectClass *gobject_class;
  gobject_class = G_OBJECT_CLASS (class);
  gobject_class->constructor = psppire_dialog_action_pp_plot_constructor;
  gobject_class->finalize = psppire_dialog_action_pp_plot_finalize;
}

static void psppire_dialog_action_pp_plot_finalize (GObject *o)
{	
}

static void
psppire_dialog_action_pp_plot_init (PsppireDialogActionPPPlot *act)
{
}
